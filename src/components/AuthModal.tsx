...
// Safely resolve the site URL for OAuth/magic-link redirects.
// NOTE: `process` does not exist in the browser bundle under Vite — referencing
// `process.env` directly here was throwing "process is not defined" on every
// Google/magic-link sign-in attempt.
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (!isSupabaseConfigured) {
      setStatusMessage({
        type: 'error',
        text: 'Sign-in is not available: Supabase is not configured for this deployment. Contact support.',
      });
    ...
    try {
      const redirectUrl = `${getSiteUrl().replace(/\/$/, '')}/auth/callback`;
  ...
  const handleMagicLinkSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        setStatusMessage({ type: 'error', text: error.message || 'Failed to send magic link. Please try again.' });
      } else {
        setStatusMessage({
          type: 'success',
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'signup') {
        ...
        if (error) {
          setStatusMessage({ type: 'error', text: error.message || 'Account creation failed. Please try again.' });
        } else {
          setStatusMessage({
            type: 'success',
        ...
        if (error) {
          setStatusMessage({ type: 'error', text: error.message || 'Sign-in failed. Please check your credentials and try again.' });
        } else {
